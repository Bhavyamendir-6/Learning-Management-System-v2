...
async def retry_quiz(
    ...
    Args:
        quiz_session_id: Optional UUID string of the quiz to retry.
                         If not provided, uses the most recent completed quiz.
        tool_context: ADK tool context (automatically provided)

    Returns:
        str: JSON string with new quiz questions focused on weak topics, or an error message
    """
    try:
        # 1. Resolve user ID
        ...
        if quiz_session_id:
            # Look up specific session via db_handler (PostgreSQL)
            # get_quiz_history returns dicts with "_id" key — find the matching one
            try:
                ...
                if not original_session:
                    return (
                        f"Quiz session '{quiz_session_id}' not found for your account."
                    )
            except Exception as e:
                return f"Error looking up quiz session '{quiz_session_id}': {str(e)}"
        else:
            original_session = await get_last_completed_quiz(user_id)

        if not original_session:
            return (
                "No completed quiz found to retry. "
                "Please complete a quiz first, then ask to retry your wrong answers."
            )
        ...
        if not document_name:
            return "Error: The original quiz session has no document name. Cannot generate retry questions."
        ...
        if not wrong_answers:
            return f"You got all questions correct in your last quiz on '{document_name}'. No retry needed!"
        ...
        if not isinstance(questions, list) or len(questions) == 0:
            return "Failed to generate retry quiz questions. Please try again."
        ...
        if tool_context:
            ...
            try:
                ...
                tool_context.state["quiz_session_id"] = quiz_session_id_new

                # Mark as retry in PostgreSQL
                try:
                    await _mark_retry(quiz_session_id_new, original_session_id)
                except Exception as e:
                    print(f"Warning: Failed to set retry metadata: {str(e)}")

            except Exception as e:
                print(f"Warning: Failed to persist retry quiz to database: {str(e)}")
        ...
        return "Failed to parse retry quiz questions from the AI response. Please try again."
    except Exception as e:
        return f"Error generating retry quiz: {str(e)}"
